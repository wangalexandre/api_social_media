...
def get_posts(current_user: int = Depends(oauth2.get_current_user)):
    ...
    cursor.execute("""SELECT p.*, u.id, u.email, u.created_at FROM public.posts p JOIN public.users u ON p.user_id = u.id """)
...
def get_post(id: int, current_user: int = Depends(oauth2.get_current_user)):
    ...
    cursor.execute("""SELECT p.*, u.id, u.email, u.created_at FROM public.posts p JOIN public.users u ON p.user_id = u.id WHERE p.id = %s """, (str(id), ))
